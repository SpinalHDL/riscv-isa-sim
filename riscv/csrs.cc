...
bool medeleg_csr_t::unlogged_write(const reg_t val) noexcept {
  const reg_t mask = 0
    ...
    | (0 << CAUSE_FETCH_ACCESS)
    | (0 << CAUSE_ILLEGAL_INSTRUCTION)
    | (1 << CAUSE_BREAKPOINT)
    | (0 << CAUSE_MISALIGNED_LOAD)
    | (0 << CAUSE_LOAD_ACCESS)
    | (0 << CAUSE_MISALIGNED_STORE)
    | (0 << CAUSE_STORE_ACCESS)
    | (1 << CAUSE_USER_ECALL)
    ...
    | (0 << CAUSE_SOFTWARE_CHECK_FAULT)
    | (0 << CAUSE_HARDWARE_ERROR_FAULT)
    ;
